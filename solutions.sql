...
SELECT books.title, editions.isbn, publishers.name AS publisher_name, stock.retail FROM books
JOIN editions ON books.id = editions.book_id
...
SELECT books.title, COUNT(editions.book_id) AS editions FROM books
JOIN editions ON books.id = editions.book_id
